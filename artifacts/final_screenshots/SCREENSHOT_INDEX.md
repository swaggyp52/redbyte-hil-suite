...
| 02 | `02_import_dialog_rigol_ds0.png` | 90 KB | 1125×960 | Import dialog with RigolDS0.csv loaded — File Metadata (1 M rows, 10 MHz, 0.100 s, 3 channels), channel mapping table **before** mapping (all `__unmapped__`), raw voltage range ±2.3 V (probe not yet scaled) | **Step 1 — Import** |
...
| 04 | `04_dataset_overview_rigol_ds0.png` | 83 KB | 2560×1351 | Overview page after import — "Rigol CSV · RigolDS0.csv", VSM/GFM analysis mode badge, explicit `Applied scale factors: v_an×100, v_bn×100, v_cn×100`, channel summary/missing-channel list, Open Analysis Workspace CTA | **Step 3 — Dataset overview** |
| 05 | `05_replay_phase_voltages.png` | 181 KB | 2560×1351 | Replay tab (all four stacked plots) — Phase-to-Neutral Voltages (v\_an/v\_bn/v\_cn ~120 V RMS after ×100 probe scale), Line-to-Line Voltages (v\_ab/v\_bc/v\_ca), Phase Currents (empty — not mapped), Frequency/Auxiliary (empty). Events panel visible on right | **Step 4 — Replay waveforms** |
...
| 14 | `14_simulation_analysis_frequency.png` | 149 KB | 2560×1351 | Replay tab for VSGFrequency\_Simulation — Same 5-event profile as shot 12 (same Pinv/p\_mech source data). Events panel prominent on right. Shows multi-file ingestion capability; does NOT show distinct frequency-domain data. | **Step 6b — Frequency simulation events** |
...
| 5 | `08_compliance_table.png` | Standards compliance cards: 6 applicable checks with 2 N/A — transparent PASS/FAIL/N/A behavior |
...
- **Events (shots 05, 06)**: DS0 replay includes an event lane entry from detector heuristics in this run, while the clearly visible step\_change event profile in shots 12/14 comes from the simulation `p_mech` waveform.
