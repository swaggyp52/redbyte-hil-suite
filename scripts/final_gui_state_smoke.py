...
This script validates replay readiness using either repo-local senior-design
inputs or generated fallback files so package installs do not depend on any
...
PRIVATE_DATA_ROOT = Path(r"C:\Users\conno\OneDrive - Gannon University\Spring 2026\Senior Design")
...
def _write_rigol_csv(path: Path, include_ch4: bool = False, rows: int = 1001) -> None:
    headers = ["Time(s)", "CH1(V)", "CH2(V)", "CH3(V)"]
    if include_ch4:
        headers.append("CH4(V)")
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for idx in range(rows):
            ...
            if include_ch4:
                row.append(f"{0.25 * math.cos(omega * t):.6f}")
            writer.writerow(row)


def _write_power_xlsx(path: Path, rows: int = 500) -> None:
    import openpyxl

    workbook = openpyxl.Workbook()
    worksheet = workbook.active
    worksheet.title = "power"
    worksheet.append(["time", "Pinv"])

    dt = 1.0 / 20_000.0
    for idx in range(rows):
        t = idx * dt
        value = 200.0 + 50.0 * math.sin(2 * math.pi * 2 * t)
        if 200 <= idx < 220:
            value = 450.0
        worksheet.append([t, value])

    workbook.save(path)


def _prepare_smoke_inputs() -> tuple[dict[str, Path], tempfile.TemporaryDirectory[str] | None]:
    if os.environ.get("REDBYTE_FORCE_SMOKE_FALLBACK") == "1":
        print("[INFO] Forced generated fallback smoke data.")
    else:
        private_paths = {
            "ds0": PRIVATE_DATA_ROOT / "RigolDS0.csv",
            "ds1": PRIVATE_DATA_ROOT / "RigolDS1.csv",
            "inv_power": PRIVATE_DATA_ROOT / "InverterPower_Simulation.xlsx",
            "vsg_freq": PRIVATE_DATA_ROOT / "VSGFrequency_Simulation.xlsx",
        }

        if all(path.exists() for path in private_paths.values()):
            print("[INFO] Using local senior-design smoke data.")
            return private_paths, None

        print("[INFO] Local senior-design smoke data not found. Generating fallback smoke data.")

    temp_dir = tempfile.TemporaryDirectory(prefix="redbyte-gui-smoke-")
    temp_root = Path(temp_dir.name)
    generated_paths = {
        "ds0": temp_root / "RigolDS0.csv",
        "ds1": temp_root / "RigolDS1.csv",
        "inv_power": temp_root / "InverterPower_Simulation.xlsx",
        "vsg_freq": temp_root / "VSGFrequency_Simulation.xlsx",
    }
    _write_rigol_csv(generated_paths["ds0"])
    _write_rigol_csv(generated_paths["ds1"], include_ch4=True)
    _write_power_xlsx(generated_paths["inv_power"])
    _write_power_xlsx(generated_paths["vsg_freq"])
    return generated_paths, temp_dir
...
def main() -> int:
    ...
    input_paths, temp_dir = _prepare_smoke_inputs()
    try:
        ...
        ds0_capsule, ds0_session = _import_capsule(input_paths["ds0"], {})
        ...
        inv_capsule, _inv_session = _import_capsule(input_paths["inv_power"], {})
        ...
        vsg_capsule, _vsg_session = _import_capsule(input_paths["vsg_freq"], {})
        ...
        # 3) DS1 CH4 contract
        mapper = ChannelMapper()
        ds1_raw = ingest_file(str(input_paths["ds1"]))
        ...
        ds1_capsule_auto, _ds1_session_auto = _import_capsule(input_paths["ds1"], {})
